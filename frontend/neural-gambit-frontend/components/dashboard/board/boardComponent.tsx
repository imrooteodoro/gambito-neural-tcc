...
export default function ChessBoard({ isDark }) {
  ...
  async function savePGNToBackend(pgn) {
    console.log("Tentando salvar PGN no backend...");
    if (!pgn) return;
    if (pgnSavedRef.current) {
      console.log("PGN já salvo para essa partida — ignorando duplicata.");
      return;
    }

    setIsSavingPGN(true);
    try {
      console.log("Enviando PGN para o backend:", pgn);
      const res = await fetch('/api/studies/insert', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
        body: JSON.stringify({ game_pgn: pgn })
      });
      if (!res.ok) {
        const text = await res.text();
        console.error("Erro ao salvar PGN no backend:", res.status, text);
      } else {
        const data = await res.json();
        console.log("PGN salvo com sucesso:", data);
        pgnSavedRef.current = true;
        setChatMessages(prev => [...prev, {
          type: 'system',
          content: 'PGN salvo nos seus estudos.',
          timestamp: new Date()
        }]);
      }
    } catch (err) {
      console.error("Falha ao enviar PGN:", err);
    } finally {
      setIsSavingPGN(false);
    }
  }
  ...
  function updateGameStatus() {
    if (game.isCheckmate()) {
      ...
      try {
        const pgn = game.pgn();
        savePGNToBackend(pgn);
      } catch (err) {
        console.error("Erro ao gerar PGN:", err);
      }
    } else if (game.isDraw()) {
      ...
      <div className="w-80 flex flex-col gap-4 overflow-y-auto">
        ...
        <div className={`${isDark ? 'bg-slate-800' : 'bg-white'} rounded-lg p-4 shadow-lg`}>
          <button
            ...
            className={`flex items-center gap-2 w-full p-2 rounded-lg mb-3 ${
                  ...
                  {Object.entries(boardThemes).map(([key, theme]) => (
                    <button
                      ...
                      className={`p-3 rounded-lg transition-all ${
                        boardTheme === key ? 'ring-2 ring-purple-600' : ''
                      }`}
                    >
        ...
        <div className={`${isDark ? 'bg-slate-800' : 'bg-white'} rounded-lg p-4 shadow-lg`}>
          ...
          <button
            onClick={resetGame}
            className={`flex items-center justify-center gap-2 w-full px-4 py-2 rounded-lg transition-all ${
              isDark ? 'bg-slate-700 hover:bg-slate-600' : 'bg-slate-200 hover:bg-slate-300'
            }`}
          >
            <RotateCcw size={18} />
            ...
            {moveHistory.map((move, index) => (
              <div
                ...
                className={`p-2 rounded ${
                  index % 2 === 0 ? (isDark ? 'bg-slate-700' : 'bg-slate-100') : ''
                }`}
              >
